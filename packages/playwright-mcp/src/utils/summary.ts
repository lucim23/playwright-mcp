 ...
 */
export function parseSnapshotElements(snapshotText: string): Map<string, number> {
 ...
 */
export function extractLandmarks(snapshotText: string): string[] {
 ...
 */
export function extractHeadings(snapshotText: string): string[] {
  const headings: string[] = [];
  const lines = snapshotText.split('\n');

  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed.startsWith('- heading')) continue;
 ...
 */
export function extractKeyText(snapshotText: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('- generic') || trimmed.startsWith('- group')) continue;
 ...
 */
export function summarizeSnapshot(snapshotText: string, options?: {
  pageTitle?: string;
  pageUrl?: string;
}): SnapshotSummary {
  ...
  for (const line of snapshotText.split('\n')) {
    if (line.trim().startsWith('- ')) totalElements++;
  }
 ...
 */
export function formatSnapshotSummary(summary: SnapshotSummary): string {
  const lines: string[] = [];
