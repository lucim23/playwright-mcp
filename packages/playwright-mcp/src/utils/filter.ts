 ...
 */
function filterByIncludeRoles(parsed: ParsedLine[], roles: string[]): Set<number> {
  const lowerRoles = roles.map(r => r.toLowerCase());
  const keepIndices = new Set<number>();

  for (let i = 0; i < parsed.length; i++) {
    const line = parsed[i];
    if (!line.isElement) continue;
 ...
 */
function filterByExcludeRoles(parsed: ParsedLine[], roles: string[]): string[] {
  const lowerRoles = roles.map(r => r.toLowerCase());
  const result: string[] = [];

  // Stack of indentation levels of currently-excluded ancestors
  const excludedIndents: number[] = [];
 ...
 */
export function filterSnapshotText(
  snapshotText: string,
  options: FilterOptions
): { text: string; meta: FilterMeta } {
...
function countElements(text: string): number {
  let count = 0;
